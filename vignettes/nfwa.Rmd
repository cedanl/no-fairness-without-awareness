...
**Aanbevolen:** Gebruik het [1cijferho project](https://github.com/cedanl/1cijferho) om je ruwe 1CHO data voor te bereiden. Dit project converteert 1CHO bestanden naar het juiste formaat (CSV met puntkomma separator) dat direct gebruikt kan worden als input voor NFWA analyse.
...
# Installeer TinyTeX voor PDF generatie (alleen eerste keer)
...
**Aanbevolen:** Voor de meeste gebruikers is `analyze_fairness()` de eenvoudigste manier om een complete NFWA-analyse uit te voeren.

```{r quickstart}
library(nfwa)

# Laad je 1CHO data (CSV bestanden met puntkomma als separator)
data_ev <- read.csv("pad/naar/jouw_EV_enriched_bestand.csv", sep = ";")
data_vakhavw <- read.csv("pad/naar/jouw_VAKHAVW_bestand.csv", sep = ";")

# Voer complete analyse uit met één functie
result <- analyze_fairness(
  data_ev = data_ev,
  data_vakhavw = data_vakhavw,
  opleidingscode = 60048,              # ISAT-opleidingscode (te vinden in EV-bestand)
  opleidingscode = 60048,
  opleidingsnaam = "Jouw Opleiding",  # Naam voor het rapport
  eoi = 2020,                          # Eerste jaar aan deze opleiding/instelling
  opleidingsvorm = "VT",               # VT (voltijd), DT (deeltijd), of DU (duaal)
  generate_pdf = TRUE,                 # Genereer PDF rapport
  cleanup_temp = TRUE                  # Ruim tijdelijke bestanden automatisch op
)

# Het PDF rapport is opgeslagen in je working directory!
# Bekijk het getransformeerde dataframe:
head(result$df)
```

**Wat doet `analyze_fairness()`?**

Deze functie voert automatisch alle stappen uit:
1. **Metadata laden** - Variabelen, sensitieve attributen, en labels
2. **Data transformeren** - Ruwe 1CHO data naar analyse-klaar formaat
3. **Modellen trainen** - Logistic Regression en Random Forest
4. **Fairness-analyse** - Checks op alle sensitieve variabelen (geslacht, vooropleiding, etc.)
5. **Visualisaties** - Dichtheidsplots en fairness-check plots
6. **PDF rapport** - Professioneel rapport met alle bevindingen (optioneel)
7. **Cleanup** - Verwijdert tijdelijke bestanden (optioneel)

**Parameters:**

- `data_ev`: 1CHO inschrijvingsdata (EV-bestand, enriched formaat)
- `data_vakhavw`: 1CHO vak/cijfer data (VAKHAVW-bestand)
- `opleidingscode`: ISAT-opleidingscode voor filtering (bijv. `60048`)
- `opleidingsnaam`: Naam van de opleiding voor het rapport (bijv. "Informatica")
- `eoi`: Examenonderdeel identificatie (eerste jaar aan opleiding)
- `opleidingsvorm`: "VT", "DT", of "DU"
- `generate_pdf`: Genereer PDF rapport (standaard: `TRUE`)
- `cleanup_temp`: Ruim tijdelijke bestanden op na PDF generatie (standaard: `TRUE`)
- `caption`: Optionele caption voor visualisaties (standaard: automatisch gegenereerd)

**Wil je meer controle?** Zie hieronder voor de stap-voor-stap workflow.

## Stap-voor-stap workflow

Een typische NFWA-analyse bestaat uit vier stappen:
...
4. **PDF rapport genereren** - Optioneel rapport met alle bevindingen
...
# Decodering van vooropleidingen en ISAT-codes is verplaatst naar het 1cijferho project.
# Gebruik het enriched EV-bestand als input.
```
...
- Decodeertabellen (dec_vopl, dec_isat) zijn verwijderd - decodering hoort in het 1cijferho project
...
df <- transform_data(
  metadata = metadata,
  opleidingscode = 60048,  # ISAT-opleidingscode (te vinden in de opleidingscode kolom)
  opleidingsvorm = "VT",   # VT = Voltijd, DT = Deeltijd, DU = Duaal
  eoi = 2020,              # Eerste jaar dat studenten aan deze opleiding/instelling konden starten
  data_ev = data_ev,
  data_vakhavw = data_vakhavw
)
...
run_nfwa(
  ...
  cutoff = cutoff,
  caption = paste0("Bron: 1CHO data | Analyse: ", format(Sys.Date(), "%B %Y"))
)
```
...
De analyse slaat resultaten op in `temp/`:

```{r results}
# Laad de conclusies
conclusions <- readRDS("temp/conclusions_list.rds")
...
```{r render_pdf}
# Genereer PDF rapport (wordt opgeslagen in working directory)
render_report(
  opleidingscode = 60048,
...
- LaTeX distributie (TinyTeX: `tinytex::install_tinytex()`)
...
### Stap 6: Tijdelijke bestanden opruimen

Na het genereren van je PDF rapport kun je de tijdelijke bestanden verwijderen:

```{r cleanup}
# Verwijder tijdelijke bestanden handmatig
cleanup_temp()

# Of automatisch tijdens PDF generatie:
render_report(
  opleidingscode = 60048,
  opleidingsnaam = "Jouw Opleiding",
  opleidingsvorm = "VT",
...
**Let op:** Zorg dat je het PDF rapport hebt gegenereerd voordat je de tijdelijke bestanden verwijdert!
...
### Complete workflow met cleanup

```{r complete_workflow}
# Aanbevolen: Complete analyse met automatische cleanup (standaard gedrag)
result <- analyze_fairness(
  data_ev = data_ev,
  data_vakhavw = data_vakhavw,
  opleidingscode = 60048,
  ...
  opleidingsvorm = "VT",
  generate_pdf = TRUE,
  cleanup_temp = TRUE  # Standaard: tijdelijke bestanden worden opgeruimd
)

# Of zonder cleanup (als je de tussentijdse bestanden wilt bewaren):
result <- analyze_fairness(
  data_ev = data_ev,
  data_vakhavw = data_vakhavw,
  opleidingscode = 60048,
  ...
  opleidingsvorm = "VT",
  generate_pdf = TRUE,
  cleanup_temp = FALSE  # Behoud tijdelijke bestanden in temp/
)
```
...
- Functiedocumentatie: `?analyze_fairness`, `?run_nfwa`, `?transform_data`, `?read_metadata`
- Package help: `?nfwa`
