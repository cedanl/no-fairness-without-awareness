...
for (var_name in names(conclusions_list)) {
  ...
  if (file.exists(fairness_file)) {
    cat("![Fairness-metrieken](", fairness_file, "){ width=48% }")
  }
  if (file.exists(density_file)) {
    cat("![Model-voorspellingen](", density_file, "){ width=48% }\n\n")
  }
  cat("\\newpage\n\n")
}
```
...
- **Causaliteit**: Deze analyse identificeert associaties, niet oorzaken.
- **Beschikbare variabelen**: Alleen geregistreerde gegevens kunnen worden geanalyseerd; belangrijke factoren zoals motivatie en thuissituatie zijn niet opgenomen.
- **Modelperformance**: Machine learning modellen zijn afhankelijk van datakwaliteit en kunnen subtiele patronen missen.
- **Groepsgrootte**: Groepen kleiner dan 15 studenten geven statistisch onbetrouwbare resultaten.
