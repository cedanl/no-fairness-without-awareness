...
Metadata definieert welke variabelen in de analyse worden gebruikt en hoe categorieën worden gelabeld.

```{r metadata}
library(nfwa)

# Lees metadata in uit Excel-bestanden
metadata <- read_metadata()

# Metadata bevat:
# - variables: Lijst van variabelen voor het model
# - df_levels: Categorie labels per variabele
# - sensitive_variables: Variabelen voor fairness-checks
# - dfapcg: APCG-verrijkingsdata (optioneel)
# - dfses: SES-verrijkingsdata (optioneel)
```

**Bestandsstructuur metadata:**

- `metadata/variabelen.xlsx`: Kolommen met variabelenamen
- `metadata/levels.xlsx`: Mapping van categoriewaarden naar labels
- `metadata/apcg.parquet`: APCG-verrijking (optioneel)
- `metadata/ses.parquet`: SES-verrijking (optioneel)
