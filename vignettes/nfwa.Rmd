...
Voor een snelle complete analyse gebruik je de `analyze_fairness()` functie:
...
result <- analyze_fairness(
  ...
  opleidingsnaam = "Jouw Opleiding",  # Pas aan
  eoi = 2020,                          # Pas aan: eerste jaar aan deze opleiding
  opleidingsvorm = "VT",              # VT, DT, of DU
  generate_pdf = TRUE,
  cleanup_temp = FALSE
)
...
Deze functie voert automatisch alle stappen uit: metadata laden, data transformeren, modellen trainen, fairness-analyse, en PDF generatie.
...
```{r complete_workflow}
# Alles in één keer, inclusief automatische cleanup
result <- analyze_fairness(
  data_ev = data_ev,
  data_vakhavw = data_vakhavw,
  opleidingsnaam = "Jouw Opleiding",
  ...
  opleidingsvorm = "VT",
  generate_pdf = TRUE,
  cleanup_temp = TRUE  # Ruim automatisch op na PDF generatie
)
```
